...
GTEST_TEST(Parser, Empty)
{
    Parser parser;

    Rt2::String text = "{}";

    Type* a = parser.parse(text.c_str(), text.size());
    EXPECT_NE(nullptr, a);
    EXPECT_EQ(a->type(), Type::OBJECT);

    text = "[]";
    Type* b = parser.parse(text.c_str(), text.size());
    EXPECT_NE(nullptr, b);
    EXPECT_EQ(b->type(), Type::ARRAY);
